...
struct PlaidLinkUIViewControllerRepresentableExampleView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            Text("Plaid Link UIViewControllerRepresentable Example")
        ...
        .sheet(isPresented: $isPresentingLink) {
            if let session = linkSession {
                PlaidLinkUIViewControllerRepresentable(session: session)
            }
        }
    }
