...
typedef NS_ENUM(NSInteger, PLKViewNameValue) {
    ...
    PLKViewNameValueProfileDataReview,
...
/// A block called when Plaid Link has finished loading.
typedef void(^PLKOnLoadHandler)(void);
...
@property (nonatomic, copy, nullable) PLKOnLoadHandler onLoad;
...
@interface PLKLayerTokenConfiguration : NSObject

@property(nonatomic, copy, readonly) NSString *token;

@property(nonatomic, copy) PLKOnSuccessHandler onSuccess;
@property(nonatomic, strong) PLKOnExitHandler onExit;
@property(nonatomic, strong) PLKOnEventHandler onEvent;
...
@interface PLKEmbeddedLinkTokenConfiguration : NSObject
...
@protocol PLKPlaidLinkSession <NSObject>

/// Controls whether the gradient background is visible.
@property (nonatomic, assign) BOOL showGradientBackground;
...
@protocol PLKPlaidLayerSession <NSObject>
...
@protocol PLKPlaidHeadlessSession <NSObject>
